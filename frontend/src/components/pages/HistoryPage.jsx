...
const HistoryPage = () => {
    ...
    useEffect(() => {
        const getSearchHistory = async () => {
            try {
                const res = await axiosInstance.get(`/search/getSearchHistory`);
    ...
    const handleDelete = async (entry) => {
		try {
			await axiosInstance.delete(`/search/removeHistory/${entry.id}`);
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3  gap-4'>
                    {searchHistory?.map((entry) =>(
                        <div key={entry.id} className='bg-gray-800 p-4 rounded flex items-start'>
                            { !entry.image ? <img src={avatar} alt="avatar" className='w-16 h-16 rounded-full object-cover mr-4' /> : 
                            <img src={SMALL_IMG_BASE_URL + entry.image} alt="Image" className='w-16 h-16 rounded-full object-cover mr-4' /> }
