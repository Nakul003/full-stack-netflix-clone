...
const HomeScreen = () => {
  ...
  const {contentType, getTrendingContent, trendingContent} = useContentStore();
    ...
    <>
      <div className='relative h-screen text-white'>
        <Navbar />
        {
          ...
          <img src={ORIGINAL_IMG_BASE_URL + trendingContent?.backdrop_path} alt="img" className='absolute top-0 left-0 w-full h-full object-cover -z-50' onLoad={()=>{setIsLoadingImg(false)}}/>

        <div className='absolute top-0 left-0 w-full h-full  -z-50 bg-black/50 '
        aria-hidden='true'/>

        <div className='absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32'>
          <div className='bg-gradient-to-b from-black/75 via-transparent to-transparent absolute w-full h-full top-0 left-0 -z-10'></div>
          ...
          <div className='flex mt-8'>
            <Link to={`/watch/${trendingContent?.id}`} className='bg-white hover:bg-white/80 text-black font-bold py-2 px-4 rounded mr-4 flex'>
